...
class DataStoreTest : Spek({
    group("accepts objects and stores them") {
        ...
        val contractorList = listOf(Contractor().apply { contractorName = "abc" }, Contractor().apply { contractorName = "cba" })
        test("contractors") {
            val subject = DataStore()
    ...
    group("ignores duplicates") {
        ...
        val contractorList = listOf(Contractor().apply { contractorName = "abc" }, Contractor().apply { contractorName = "cba" }, Contractor().apply { contractorName = "cba" })
        test("contractors") {
            val subject = DataStore()
            subject.ingest(contractorList)
            expect(subject.contractors).to.have.size(2)
